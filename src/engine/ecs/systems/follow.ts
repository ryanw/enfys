...
export class FollowSystem extends System {
	...
	override async tick(dt: number, world: World) {
		const entities = world.entitiesWithComponents([FollowComponent, TransformComponent]);
		for (const entity of entities) {
			const { target, axis } = world.getComponent(entity, FollowComponent)!;
			if (!target) continue;


			const srcTra = world.getComponent(target, TransformComponent)!;
			const dstTra = world.getComponent(entity, TransformComponent)!;

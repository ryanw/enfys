...
import renderSunShaderSource from './render_sun.wgsl';
...
export class RenderSunPipeline extends MaterialPipeline {
	...
	constructor(gfx: Gfx, source?: string) {
		...
		const shader = device.createShaderModule({
			label: 'RenderSunPipeline Shader',
			code: source || renderSunShaderSource
		});

		const cameraBindGroupLayout = device.createBindGroupLayout({
			label: 'RenderSunPipeline Bind Group Layout',
			entries: [
				// Camera
				{
					binding: 0,
	...
	drawBatch(encoder: GPUCommandEncoder, pawns: Array<Pawn<SimpleMesh>>, camera: Camera, target: GBuffer) {
		...
		for (const pawn of pawns) {
			...
			const bindGroup = device.createBindGroup({
				label: 'RenderSunPipeline Pass Bind Group',

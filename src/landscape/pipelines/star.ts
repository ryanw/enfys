...
import shaderSource from './star.wgsl';
import { UniformBuffer } from 'engine/uniform_buffer';
import { Point3 } from 'engine/math';

export class StarPipeline extends Pipeline {
	...
	constructor(gfx: Gfx) {
		...
		const shader = device.createShaderModule({ label: 'StarPipeline Shader', code: shaderSource });
	...
	async createInstanceBuffer(position: Point3, radius: number, density: number, seed: number): Promise<[GPUBuffer, number]> {
		...
		const instanceByteSize = 5 * 4;// FIXME vec3f derive from type? TransformInstance
		...
		const enc = device.createCommandEncoder({ label: 'StarPipeline Command Encoder' });
		const pass = enc.beginComputePass({ label: 'StarPipeline Compute Pass' });

		const bindGroup = device.createBindGroup({
			label: 'StarPipeline Bind Group',
